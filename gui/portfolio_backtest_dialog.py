# # gui/portfolio_backtest_dialog.py
...
#     QTabWidget, QWidget, QDoubleSpinBox, QSpinBox, QMessageBox, QAbstractScrollArea
# )
...
# from core.commission import FixedCommission, CommissionModel
...
#         table_layout.addWidget(QLabel("Стратегии в портфеле:"))

#         self.strategy_table = QTableWidget()
#         self.strategy_table.setMinimumHeight(150)          # видно несколько строк
...
#         self.strategy_table.setHorizontalHeaderLabels(["Класс", "Имя", "Тикер(ы)", "Доля (%)"])
#         self.strategy_table.horizontalHeader().setStretchLastSection(True)
...
#         self.portfolio_metrics_table.setMinimumHeight(150)          # видно несколько строк
#         self.portfolio_metrics_table.horizontalHeader().setStretchLastSection(True)
#         self.portfolio_metrics_table.verticalHeader().setVisible(False)
...
#         # Вкладки для индивидуальных стратегий будут добавляться динамически
...
#         # Тикер (показываем подписки по умолчанию)
#         cls = STRATEGY_REGISTRY[class_combo.currentText()]
#         # Создаём временный экземпляр, чтобы узнать subscriptions
#         # Но конструктор требует event_bus и order_manager, дадим моки
#         from core.mocks import MockEventBus, MockOrderManager
#         try:
#             temp_instance = cls(name="tmp", event_bus=MockEventBus(), order_manager=MockOrderManager())
#             tickers = ", ".join(sorted({s for s, _ in temp_instance.subscriptions}))
#         except:
#             tickers = "?"
...
#         share_spin.setValue(100 // (row+1))  # равномерно
#         self.strategy_table.setCellWidget(row, 3, share_spin)
...
#             share_spin = self.strategy_table.cellWidget(row, 3)
#             if not all([class_combo, name_edit, share_spin]):
#                 continue
...
#             if not cls:
#                 continue
#             # Получаем subscriptions через временный экземпляр
#             from core.mocks import MockEventBus, MockOrderManager
...
#         for name, tab in self.individual_tabs.items():
#             self.results_tabs.removeTab(self.results_tabs.indexOf(tab))
#         self.individual_tabs.clear()

#         # Создаём новые вкладки для каждой стратегии
