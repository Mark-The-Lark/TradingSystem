...
class PortfolioBacktestDialog(QDialog):
    ...
    def on_start(self):
        ...
        for row in range(self.strategy_table.rowCount()):
            ...
            if ticker_edit is None:
                try:
                    temp = class_combo.currentText(name="tmp", event_bus=MockEventBus(), order_manager=MockOrderManager())
                    ticker_edit = ", ".join(sorted({s for s, _ in temp.subscriptions}))
                except:
                    self._show_error("Невозможно определить тикеры")
            # ticker_edit = self.strategy_table.cellWidget(row, 2)
        ...
        def run():
            try:
                ...
                # QMetaObject.invokeMethod(self, "_show_results", Qt.ConnectionType.QueuedConnection, Q_ARG(dict, result))
    ...
    def _show_results(self, result):
        ...
        if self.results_tabs.count() > 0:
            self.results_tabs.setCurrentIndex(self.results_tabs.count() - 1)  # или 0
        self.results_tabs.update()
        print("=== Диагностика вкладок ===")
        for i in range(self.results_tabs.count()):
            tab = self.results_tabs.widget(i)
            text = self.results_tabs.tabText(i)
            parent = tab.parent()
            is_window = tab.isWindow()
            print(f"Вкладка {i} (\"{text}\"): родитель={parent}, isWindow={is_window}")
